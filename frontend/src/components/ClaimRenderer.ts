...
export class ClaimRenderer {
    ...
    static renderClaim(claim: Claim): string {
        const containerClass = this.getContainerClass(claim.ursLabel);
        const textClass = this.getTextClass(claim.ursLabel);

        let html = `
      <div class="${containerClass}">
        <div class="flex items-start gap-2 mb-2">
          ${UrsBadge.render(claim.ursLabel, claim.urs)}
          ${claim.isInference ?
        ...
        <div class="mt-2 text-xs space-y-1">
          ${Object.entries(breakdown)
              ...
              <div class="flex justify-between">
                ...
                <span class="font-semibold">${(value as number).toFixed(2)}</span>
              </div>
            `).join('')}
        </div>
      </details>
