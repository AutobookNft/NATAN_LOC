...
<x-natan.layout title="{{ __('users.edit') }}">
            ...
            <div class="flex-1 overflow-y-auto">
                <div class="container mx-auto px-3 sm:px-4 py-4 sm:py-6 max-w-4xl">
                    ...
                    <div class="mb-3 flex items-center gap-1.5 text-xs sm:text-sm">
                        ...
                        <span class="text-gray-400">/</span>
                        <span class="text-gray-700">{{ __('users.edit') }}</span>
                    </div>

                    <h1 class="mb-4 text-xl sm:text-2xl font-bold text-gray-900">{{ __('users.edit') }}: {{ $user->name }}</h1>
                    ...
                    <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-6">
                        ...
                        {{-- Password (optional) --}}
                        <div>
                            <label for="password" class="block text-sm font-medium text-gray-700 mb-1">{{ __('users.password') }} ({{ __('users.optional', ['default' => 'opzionale']) }})</label>
                            <input type="password" id="password" name="password"
                                minlength="8"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1B365D] focus:border-transparent">
                            <p class="mt-1 text-xs text-gray-500">Lascia vuoto per non modificare la password</p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="space-y-2 border border-gray-300 rounded-lg p-4 max-h-64 overflow-y-auto">
                                @php
                                    ...
                                    $oldRoles = old('roles', $userRoleNames);
                                @endphp
