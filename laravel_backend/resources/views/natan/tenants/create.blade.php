...
<x-natan.layout title="{{ __('tenants.create') }}">
            ...
            <div class="flex-1 overflow-y-auto">
                <div class="container mx-auto px-3 sm:px-4 py-4 sm:py-6 max-w-4xl">
                    ...
                    <div class="mb-3 flex items-center gap-1.5 text-xs sm:text-sm">
                        ...
                        <span class="text-gray-400">/</span>
                        <span class="text-gray-700">{{ __('tenants.create') }}</span>
                    </div>

                    <h1 class="mb-4 text-xl sm:text-2xl font-bold text-gray-900">{{ __('tenants.create') }}</h1>
                    ...
                    <form method="POST" action="{{ route('tenants.store') }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="entity_type" name="entity_type" required
                                ...
                                <option value="pa" {{ old('entity_type') === 'pa' ? 'selected' : '' }}>{{ __('tenants.entity_type_pa') }}</option>
                                <option value="company" {{ old('entity_type') === 'company' ? 'selected' : '' }}>{{ __('tenants.entity_type_company') }}</option>
                                <option value="public_entity" {{ old('entity_type') === 'public_entity' ? 'selected' : '' }}>{{ __('tenants.entity_type_public_entity') }}</option>
                                <option value="other" {{ old('entity_type') === 'other' ? 'selected' : '' }}>{{ __('tenants.entity_type_other') }}</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="address" name="address" rows="3"
                                ...
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1B365D] focus:border-transparent">{{ old('address') }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input type="date" id="subscription_ends_at" name="subscription_ends_at" value="{{ old('subscription_ends_at') }}"
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1B365D] focus:border-transparent">
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea id="notes" name="notes" rows="4"
                                class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1B365D] focus:border-transparent">{{ old('notes') }}</textarea>
                        </div>
