 ...
 * 1. Subdomain detection (es: firenze.natan.loc → cerca tenants.slug)
...
class TenantResolver
{
    ...
    public static function resolve(): ?int
    {
        ...
        // Esempio: firenze.natan.loc → cerca slug="firenze" in tenants
        ...
        if ($headerTenantId = request()->header('X-Tenant')) {
            ...
            $tenant = is_numeric($headerTenantId)
                ? Tenant::find($headerTenantId)
                : Tenant::where('slug', $headerTenantId)->first();
